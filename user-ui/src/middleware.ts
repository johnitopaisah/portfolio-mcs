 ...
 *    so kubectl logs shows client interactions (IP, method, path, status,
...
// Paths that are Next.js internals — log them only in dev, skip in prod
...
export async function middleware(request: NextRequest) {
  ...
  // ── Page requests ────────────────────────────────────────────
  // Skip logging Next.js internals (_next/static, _next/image, etc.)
  ...
  const ms = Date.now() - start;
  log(request, 200, ms); // page status resolved by the renderer; 200 is the proxy-level status
  return response;
}

// ── Logger ────────────────────────────────────────────────────
// Output format (matches API morgan "combined"):
//   [user-ui] GET /projects 200 12ms :: 1.2.3.4 :: Mozilla/5.0 ...
function log(request: NextRequest, status: number, ms: number) {
  ...
  // Colour status code like morgan: green 2xx, yellow 3xx, red 4xx/5xx
...
// ── Matcher ───────────────────────────────────────────────────
// Catch all routes EXCEPT Next.js static asset chunks (_next/static,
// _next/image) — those are served directly and don't need middleware.
export const config = {
  matcher: [
