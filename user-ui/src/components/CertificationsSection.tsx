...
const ACCENT: Record<AccentKey, { card: string; icon: string; text: string; verify: string }> = {
  aws:       { card: 'hover:border-yellow-500/40',  icon: 'bg-yellow-500/10 border-yellow-500/30',  text: 'text-yellow-300', verify: 'bg-yellow-500/10 border-yellow-500/30 text-yellow-300 hover:bg-yellow-500/20' },
  cncf:      { card: 'hover:border-cyan-500/40',    icon: 'bg-cyan-500/10 border-cyan-500/30',      text: 'text-cyan-300',   verify: 'bg-cyan-500/10 border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/20' },
  google:    { card: 'hover:border-blue-500/40',    icon: 'bg-blue-500/10 border-blue-500/30',      text: 'text-blue-300',   verify: 'bg-blue-500/10 border-blue-500/30 text-blue-300 hover:bg-blue-500/20' },
  microsoft: { card: 'hover:border-violet-500/40',  icon: 'bg-violet-500/10 border-violet-500/30',  text: 'text-violet-300', verify: 'bg-violet-500/10 border-violet-500/30 text-violet-300 hover:bg-violet-500/20' },
  default:   { card: 'hover:border-violet-500/40',  icon: 'bg-violet-500/10 border-violet-500/30',  text: 'text-violet-300', verify: 'bg-violet-500/10 border-violet-500/30 text-violet-300 hover:bg-violet-500/20' },
};
  ...
  return (
    <section id="certifications" className="relative overflow-hidden">
      ...
      <div className="absolute top-0 right-1/4 w-96 h-96 pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(6,182,212,0.06) 0%, transparent 70%)' }} aria-hidden />
            ...
            return (
              <div key={cert.id}
                className={`group flex flex-col items-center text-center p-5 rounded-2xl
                  ...
                  bg-zinc-900/60 border border-zinc-800 ${accent.card}`}>
                ...
                <div className="mb-3 relative">
                  ...
                  {/* Valid green tick */}
                  {isValid && (
                    <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-green-500 border-2 border-zinc-900 flex items-center justify-center">
                      <span style={{ fontSize: '8px', color: '#fff', lineHeight: 1 }}>✓</span>
                    </div>
                  )}
                </div>

                <h3 className="text-white font-semibold text-sm leading-snug mb-1 line-clamp-2">{cert.name}</h3>
